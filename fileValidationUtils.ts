...
export const isValidEvidenceTypeText = (value: string, pattern: string): boolean => {
  try {
...
export const validateFileTypes = async (files: File[], allowedLabel: string, allowedExtensions: string): Promise<FileValidationResult> => {
  const valid: File[] = [];
